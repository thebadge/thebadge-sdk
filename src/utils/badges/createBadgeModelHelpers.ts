...
export async function createAndUploadBadgeModelMetadata(
  ...
  const badgeModelMetadataIPFSUploaded = await uploadToIPFS<BadgeModelMetadata<BackendFileUpload>>({
    attributes: {
      name: badgeModelName,
      description: badgeModelDescription,
      image: badgeModelLogoUri,
      external_link: `${THE_BADGE_DAPP_URL}/explore`,
      attributes: [
        {
...
//     badgeModelCriteriaFile.base64File = await transformDeltaToPDF(badgeModelCriteria.criteriaDeltaText.delta)
//   }
//
...
//   )
//
//   const registrationIPFSUploaded = await uploadToIPFS<KlerosListStructure>({
//     attributes: registration,
//     filePaths: ['fileURI', 'metadata.logoURI'],
//     needKlerosPath: true,
//   })
//
//   const clearingIPFSUploaded = await uploadToIPFS<KlerosListStructure>({
//     attributes: clearing,
//     filePaths: ['fileURI', 'metadata.logoURI'],
//     needKlerosPath: true,
//   })
...
//     registrationIPFSHash: convertHashToValidIPFSKlerosHash(registrationIPFSUploaded.result?.ipfsHash),
//   }
// }
//
...
//   const challengeDurationInSeconds = challengePeriodDuration * 24 * 60 * 60
//
...
//         [submissionDeposit, removalDeposit, challengeSubmissionRequestDeposit, challengeRemovalRequestDeposit], // baseDeposits:
